...
xml_add_child(l2[1], "ref-type") # how to add a node inside a specific node (i.e., add a node to l2[1] and not the other l2 nodes)

xml2::xml_children(l2)
cat(as.character(xml2::as_xml_document(real))) # sanity check

l3 <- xml2::xml_children(l2)
xml_set_attr(l3[1], "reference", names(record_list)[1]) # how to create an attribute and set its value
cat(as.character(xml2::as_xml_document(real))) # sanity check
